...
class Bank
{
    ...
    public:
    double monthly_interest_rate,monthly_interest,deposit,withdrawal;
    int update;
    int acc_no;
    void create_account();
    void read_account();
...
void Bank::delete_accounts()
{
  ...
  virtual void monthlyProc()
  {
    ...
    {
     service_charges = service_charges + 1 *(number_of_withdrawals - 4);
     monthly_service_charges = monthly_service_charges + service_charges;
    } //baki xa
    Saving_acc::monthlyProc();
    if(balance < 25)
    {
      status = INACTIVE;
    }
...
class Checking_account()  
{
  class Checking_acc : public Bank
  {
    virtual void makeWithdrawal(double wd)
    {
      ...
      {
        Checking_acc::makeWithdrawal(wd);
      }
